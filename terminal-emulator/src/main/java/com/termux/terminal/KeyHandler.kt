...
object KeyHandler {
    ...
    fun getCode(keyCode: Int, keyMode: Int, cursorApp: Boolean, keypadApplication: Boolean): String? {
        when (keyCode) {
            ...
            KeyEvent.KEYCODE_DPAD_UP -> return if (keyMode == 0) if (cursorApp) "\u001bOA" else "\u001b[A" else transformForModifiers("\u001b[1", keyMode, 'A')
            KeyEvent.KEYCODE_DPAD_DOWN -> return if (keyMode == 0) if (cursorApp) "\u001bOB" else "\u001b[B" else transformForModifiers("\u001b[1", keyMode, 'B')
            KeyEvent.KEYCODE_DPAD_RIGHT -> return if (keyMode == 0) if (cursorApp) "\u001bOC" else "\u001b[C" else transformForModifiers("\u001b[1", keyMode, 'C')
            KeyEvent.KEYCODE_DPAD_LEFT -> return if (keyMode == 0) if (cursorApp) "\u001bOD" else "\u001b[D" else transformForModifiers("\u001b[1", keyMode, 'D')
            KeyEvent.KEYCODE_MOVE_HOME ->                 // Note that KEYCODE_HOME is handled by the system and never delivered to applications.
                ...
                return if (keyMode == 0) if (cursorApp) "\u001bOH" else "\u001b[H" else transformForModifiers("\u001b[1", keyMode, 'H')
            KeyEvent.KEYCODE_MOVE_END -> return if (keyMode == 0) if (cursorApp) "\u001bOF" else "\u001b[F" else transformForModifiers("\u001b[1", keyMode, 'F')
            KeyEvent.KEYCODE_F1 -> return if (keyMode == 0) "\u001bOP" else transformForModifiers("\u001b[1", keyMode, 'P')
            KeyEvent.KEYCODE_F2 -> return if (keyMode == 0) "\u001bOQ" else transformForModifiers("\u001b[1", keyMode, 'Q')
            KeyEvent.KEYCODE_F3 -> return if (keyMode == 0) "\u001bOR" else transformForModifiers("\u001b[1", keyMode, 'R')
            KeyEvent.KEYCODE_F4 -> return if (keyMode == 0) "\u001bOS" else transformForModifiers("\u001b[1", keyMode, 'S')
            ...
            KeyEvent.KEYCODE_DEL -> {
                ...
                return prefix + if (keyMode and KEYMOD_CTRL == 0) "\u007F" else "\u0008"
            }
            KeyEvent.KEYCODE_NUM_LOCK -> return "\u001bOP"
            KeyEvent.KEYCODE_SPACE ->                 // If ctrl is not down, return null so that it goes through normal input processing (which may e.g. cause a
                ...
                return if (keyMode and KEYMOD_CTRL == 0) null else "\u0000"
            KeyEvent.KEYCODE_TAB ->                 // This is back-tab when shifted:
                return if (keyMode and KEYMOD_SHIFT == 0) "\u0009" else "\u001b[Z"
            KeyEvent.KEYCODE_ENTER -> return if (keyMode and KEYMOD_ALT == 0) "\r" else "\u001b\r"
            KeyEvent.KEYCODE_NUMPAD_ENTER -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'M') else "\n"
            KeyEvent.KEYCODE_NUMPAD_MULTIPLY -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'j') else "*"
            KeyEvent.KEYCODE_NUMPAD_ADD -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'k') else "+"
            KeyEvent.KEYCODE_NUMPAD_COMMA -> return ","
            KeyEvent.KEYCODE_NUMPAD_DOT -> return if (keypadApplication) "\u001bOn" else "."
            KeyEvent.KEYCODE_NUMPAD_SUBTRACT -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'm') else "-"
            KeyEvent.KEYCODE_NUMPAD_DIVIDE -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'o') else "/"
            KeyEvent.KEYCODE_NUMPAD_0 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'p') else "0"
            KeyEvent.KEYCODE_NUMPAD_1 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'q') else "1"
            KeyEvent.KEYCODE_NUMPAD_2 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'r') else "2"
            KeyEvent.KEYCODE_NUMPAD_3 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 's') else "3"
            KeyEvent.KEYCODE_NUMPAD_4 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 't') else "4"
            KeyEvent.KEYCODE_NUMPAD_5 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'u') else "5"
            KeyEvent.KEYCODE_NUMPAD_6 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'v') else "6"
            KeyEvent.KEYCODE_NUMPAD_7 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'w') else "7"
            KeyEvent.KEYCODE_NUMPAD_8 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'x') else "8"
            KeyEvent.KEYCODE_NUMPAD_9 -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'y') else "9"
            KeyEvent.KEYCODE_NUMPAD_EQUALS -> return if (keypadApplication) transformForModifiers("\u001bO", keyMode, 'X') else "="
        }
        return null
    }
