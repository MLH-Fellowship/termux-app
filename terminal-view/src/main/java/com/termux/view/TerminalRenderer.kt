...
class TerminalRenderer(val mTextSize: Int, val mTypeface: Typeface?) {
    ...
    fun render(mEmulator: TerminalEmulator, canvas: Canvas, topRow: Int,
        ...
        for (row in topRow until endRow) {
            ...
            while (column < columns) {
                ...
                val codePoint: Int = (if (charIsHighsurrogate) Character.toCodePoint(charAtIndex, line[currentCharIndex + 1]) else charAtIndex).toInt()
                val codePointWcWidth = width(codePoint)
                val insideCursor = column >= selx1 && column <= selx2 || cursorX == column || codePointWcWidth == 2 && cursorX == column + 1
                ...
                currentCharIndex += charsForCodePoint
                while (currentCharIndex < charsUsedInLine && width(line, currentCharIndex) <= 0) {
