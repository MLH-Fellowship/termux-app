...
class TerminalView(context: Context, attributes: AttributeSet?) : View(context, attributes) {
    ...
    override fun computeVerticalScrollOffset(): Int {
        return if (mEmulator == null) 1 else mEmulator!!.screen.activeRows + mTopRow - mEmulator!!.mRows
    }
    ...
    fun sendMouseEventCode(e: MotionEvent, button: Int, pressed: Boolean) {
        ...
        var y = ((e.y - mRenderer!!.mFontLineSpacingAndAscent) / mRenderer!!.mFontLineSpacing).toInt() + 1
    ...
    override fun onTouchEvent(ev: MotionEvent): Boolean {
        ...
        if (mIsSelectingText) {
            ...
            if (ev.isButtonPressed(MotionEvent.BUTTON_SECONDARY)) {
                ...
                if (clipData != null) {
                    val paste = clipData.getItemAt(0).coerceToText(context)
                    if (!TextUtils.isEmpty(paste)) mEmulator!!.paste(paste.toString())
                }
            } else if (mEmulator!!.isMouseTrackingActive) { // BUTTON_PRIMARY.
                when (ev.action) {
                    MotionEvent.ACTION_DOWN, MotionEvent.ACTION_UP -> sendMouseEventCode(ev, TerminalEmulator.MOUSE_LEFT_BUTTON, ev.action == MotionEvent.ACTION_DOWN)
                    MotionEvent.ACTION_MOVE -> sendMouseEventCode(ev, TerminalEmulator.MOUSE_LEFT_BUTTON_MOVED, true)
                }
                return true
            }
        }
    ...
    fun inputCodePoint(codePoint: Int, controlDownFromEvent: Boolean, leftAltDownFromEvent: Boolean) {
    ...
    fun startSelectingText(ev: MotionEvent) {
        ...
        val cy = ((ev.y + SELECT_TEXT_OFFSET_Y) / mRenderer!!.mFontLineSpacing).toInt() + mTopRow
        mSelX2 = cx
        mSelX1 = mSelX2
        mSelY2 = cy
        mSelY1 = mSelY2
        ...
        if (" " != screen.getSelectedText(mSelX1, mSelY1, mSelX1, mSelY1)) {
            ...
            while (mSelX2 < mEmulator!!.mColumns - 1 && "" != screen.getSelectedText(mSelX2 + 1, mSelY1, mSelX2 + 1, mSelY1)) {
                mSelX2++
            }
        }
    ...
    private val text: CharSequence
        private get() = mEmulator!!.screen.getSelectedText(0, mTopRow, mEmulator!!.mColumns, mTopRow + mEmulator!!.mRows)
     ...
     */
    private interface CursorController : OnTouchModeChangeListener {
        /**
    ...
    private inner class HandleView(private val mController: CursorController, orientation: Int) : View(this@TerminalView.context) {
        ...
        public override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
            setMeasuredDimension(mDrawable!!.intrinsicWidth,
        ...
        private fun checkChangedOrientation(posX: Int, force: Boolean) {
            ...
            if (posX - mHandleWidth < clip.left) {
                changeOrientation(Companion.RIGHT)
            } else if (posX + mHandleWidth > clip.right) {
                changeOrientation(Companion.LEFT)
            } else {
        ...
        init {
            mContainer = PopupWindow(this@TerminalView.context, null,
    ...
    inner class SelectionModifierCursorController internal constructor() : CursorController {
        ...
        override fun show() {
            ...
            val callback: ActionMode.Callback = object : ActionMode.Callback {
                override fun onCreateActionMode(mode: ActionMode, menu: Menu): Boolean {
                    val show = MenuItem.SHOW_AS_ACTION_ALWAYS or MenuItem.SHOW_AS_ACTION_WITH_TEXT
                    val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
                ...
                override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
                    ...
                    when (item.itemId) {
                        1 -> {
                            val selectedText = mEmulator!!.getSelectedText(mSelX1, mSelY1, mSelX2, mSelY2)!!.trim { it <= ' ' }
            ...
            mActionMode = startActionMode(object : ActionMode.Callback2() {
                ...
                override fun onGetContentRect(mode: ActionMode, view: View, outRect: Rect) {
                    ...
                    val y1 = Math.round((mSelY1 - 1 - mTopRow) * mRenderer!!.mFontLineSpacing.toFloat())
                    val y2 = Math.round((mSelY2 + 1 - mTopRow) * mRenderer!!.mFontLineSpacing.toFloat())
                    if (x1 > x2) {
                        val tmp = x1
                        x1 = x2
                        x2 = tmp
        ...
        init {
            mStartHandle = HandleView(this, HandleView.Companion.LEFT)
            mEndHandle = HandleView(this, HandleView.Companion.RIGHT)
